...
> = ({ className = "" }) => {
  ...
  return (
    ...
    >
      ...
      <div className="self-stretch grid items-start grid-cols-[repeat(4,_minmax(240px,_1fr))] shrink-0 text-xl text-[#424242] lg:[grid-row-gap:20px] lg:justify-center lg:grid-cols-[repeat(2,_minmax(240px,_416px))] mq750:grid-cols-[minmax(240px,_1fr)]">
        {recCard1Items.map((item, index) => (
          <RecCard1
            key={index}
            bali={item.bali}
            d3N={item.d3N}
            discountValue1={item.discountValue1}
          />
        ))}
      </div>
      <div className="w-[199.9px] rounded-[31px] bg-background-backgroundprimary hidden items-start gap-2.5 whitespace-nowrap shrink-0 text-right text-lg text-[#1262ae]">
